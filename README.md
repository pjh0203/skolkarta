...
- **分布图**：当前指标的分数分布。两种视图——①**直方图**：全国（灰）与当前筛选（绿）叠加，标出中位数与前 2% / 10% / 25% 分位线，选中某校显示「该校位于前 X%」；②**箱线对比**：每个所选 kommun 一行（未选时对比全国/公立/私立），盒=25–75 分位、中线=中位、须=10–90 分位。SALSA 指标额外标出 0（预期）参考线。
- **可分享链接**：指标、区域、项目、Tab、选中校等都写入 URL（`?m=…&k=…&t=dist`），复制链接即可还原同一视图。
